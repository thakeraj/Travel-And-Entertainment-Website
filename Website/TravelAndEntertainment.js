...
$(document).ready(function(){
	...
	function getNextData(next_page_token) {
		...
		ajaxRestCall('http://tne-env-prod.us-west-1.elasticbeanstalk.com/getNext20Results', 'GET', token, getNearbyPlaces);
	}
			...
			if (status === google.maps.places.PlacesServiceStatus.OK) {
				...
				$("#reviewsOptionsList button").on('click', function(){
					...
					if(selText == 'Yelp Reviews') {
						...
						ajaxRestCall('http://tne-env-prod.us-west-1.elasticbeanstalk.com/getYelpReviews', 'GET', myFields, displayYelpReviews);
					} else {
...
function displayPlaceDetailsDiv(placeDetails) {
	...
	if(placeDetails.opening_hours) {
		...
		$('#modal-body').html(populateModalTable(placeDetails.opening_hours ,placeDetails.utc_offset));
	}
...
function loadMap(directionsDisplay, directionsService, locationLatitude, locationLongitude) {
	...
	$('#customizedSearch').on('click', function(e) {
		...
		ajaxRestCall('http://tne-env-prod.us-west-1.elasticbeanstalk.com/getNearByPlaces', 'GET', myFields, getNearbyPlaces);
	});
	...
	function getLocalStorageData() {
		if (typeof(Storage) !== "undefined") {
			if(!localStorage.getItem("csciRaj571")) {
				localStorage.setItem("csciRaj571", "");	
			}
			return localStorage.getItem("csciRaj571");
		} else {
			console.log("Sorry, your browser does not support Web Storage...");
		}
	}
	function saveLocalStorageData(data) {
		localStorage.setItem("csciRaj571", data);	
	}
	loadCategories();
	loadCurrentLocation();
});
